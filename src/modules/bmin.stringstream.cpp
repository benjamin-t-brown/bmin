...
StringStream& StringStream::operator<<(unsigned int value) {
  ...
  if (len > 0) {
    _buf.append(digits, static_cast<std::size_t>(len));
  }
  return *this;
}

StringStream& StringStream::operator<<(std::size_t value) {
  if (value == 0) {
    _buf.append("0");
    return *this;
  }
  ...
  for (std::size_t l = 0, r = i - 1; l < r; ++l, --r) {
    char tmp = digits[l];
    digits[l] = digits[r];
    digits[r] = tmp;
  }
