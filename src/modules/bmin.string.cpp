...
void bytesCopy(char* dst, const char* src, std::size_t n) {
...
String stringFromFormat(const char* fmt, ...) {
  ...
  if (static_cast<std::size_t>(len) >= sizeof(buf)) {
    std::size_t cap = static_cast<std::size_t>(len) + 1;
    ...
    va_end(args2);
    String out;
    out.append(big, static_cast<std::size_t>(len));
    delete[] big;
    return out;
  }
  return String(buf);
...
void String::growToFit(std::size_t minCapacity) {
...
String::String(std::size_t count, char c) {
  if (count == 0) {
    return;
  }
  _size = count;
...
String& String::append(const char* s, std::size_t len) {
  if (!s || len == 0) {
    return *this;
  }
...
String& String::insert(std::size_t pos, const char* s, std::size_t len) {
  if (pos > _size) {
    pos = _size;
  }
  if (!s || len == 0) {
...
String& String::insert(std::size_t pos, const String& s) {
...
String& String::erase(std::size_t pos, std::size_t count) {
  ...
  std::size_t maxRemove = _size - pos;
  if (count > maxRemove) {
    count = maxRemove;
  }
  std::size_t tailStart = pos + count;
  std::size_t tailLen = _size - tailStart;
...
SliceBounds sliceBounds(std::size_t size, std::ptrdiff_t start, std::ptrdiff_t end, bool hasEnd) {
  std::ptrdiff_t len = static_cast<std::ptrdiff_t>(size);
  std::ptrdiff_t s = start;
  std::ptrdiff_t e = hasEnd ? end : len;
  ...
  bounds.start = static_cast<std::size_t>(s);
  bounds.end = static_cast<std::size_t>(e);
  return bounds;
}
...
String String::slice(std::ptrdiff_t start) const {
  SliceBounds bounds = sliceBounds(_size, start, 0, false);
  if (bounds.start >= bounds.end) {
    return String();
  }
  String out;
  std::size_t len = bounds.end - bounds.start;
  out.reserve(len);
...
String String::slice(std::ptrdiff_t start, std::ptrdiff_t end) const {
...
std::string_view String::sliceView(std::ptrdiff_t start) const {
  SliceBounds bounds = sliceBounds(_size, start, 0, false);
  if (bounds.start >= bounds.end) {
...
std::string_view String::sliceView(std::ptrdiff_t start, std::ptrdiff_t end) const {
  SliceBounds bounds = sliceBounds(_size, start, end, true);
  if (bounds.start >= bounds.end) {
    return std::string_view();
  }
...
std::size_t String::find(const char* needle, std::size_t start) const {
  if (!needle || !*needle || start >= _size) {
    return npos;
  }
  std::size_t nlen = cstrLen(needle);
  if (nlen == 0 || start + nlen > _size) {
    return npos;
  }
  for (std::size_t i = start; i + nlen <= _size; ++i) {
    bool match = true;
...
bool String::startsWith(const char* prefix) const {
  ...
  std::size_t plen = cstrLen(prefix);
  if (plen > _size) {
    return false;
  }
...
bool String::endsWith(const char* suffix) const {
  ...
  std::size_t slen = cstrLen(suffix);
  if (slen > _size) {
    return false;
  }
...
String String::fromInt(int value) {
  ...
  for (std::size_t l = digitsStart, r = i - 1; l < r; ++l, --r) {
    char tmp = buf[l];
    buf[l] = buf[r];
    buf[r] = tmp;
  }
